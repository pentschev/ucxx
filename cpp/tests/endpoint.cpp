...
TEST_F(EndpointTest, StoppingRejectRequests)
{
  ...
  EXPECT_THROW(std::ignore = ep->amSend(tmp.data(), tmp.size() * sizeof(int), UCS_MEMORY_TYPE_HOST),
               ucxx::RejectedError);
  EXPECT_THROW(std::ignore = ep->tagSend(tmp.data(), tmp.size() * sizeof(int), ucxx::Tag{0}),
               ucxx::RejectedError);
  EXPECT_THROW(std::ignore = ep->streamRecv(tmp.data(), tmp.size() * sizeof(int)),
               ucxx::RejectedError);
  EXPECT_THROW(std::ignore = ep->streamSend(tmp.data(), tmp.size() * sizeof(int)),
               ucxx::RejectedError);

  {
    ...
    EXPECT_THROW(std::ignore = ep->memPut(tmp.data(), tmp.size() * sizeof(int), remoteKey),
                 ucxx::RejectedError);
    EXPECT_THROW(
      std::ignore = ep->memPut(
        tmp.data(), tmp.size() * sizeof(int), remoteKey->getBaseAddress(), remoteKey->getHandle()),
      ucxx::RejectedError);
    EXPECT_THROW(std::ignore = ep->memGet(tmp.data(), tmp.size() * sizeof(int), remoteKey),
                 ucxx::RejectedError);
    EXPECT_THROW(
      std::ignore = ep->memGet(
        tmp.data(), tmp.size() * sizeof(int), remoteKey->getBaseAddress(), remoteKey->getHandle()),
      ucxx::RejectedError);
  }

  {
    std::vector<void const*> buffers{tmp.data()};
    std::vector<size_t> sizes{tmp.size()};
    std::vector<int> isCUDA{false};
    EXPECT_THROW(std::ignore = ep->tagMultiSend(buffers, sizes, isCUDA, ucxx::Tag{0}),
                 ucxx::RejectedError);
  }
}
